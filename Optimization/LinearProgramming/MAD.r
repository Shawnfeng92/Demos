...
mMAD.p$obj <- c(rep(0, mMAD.p$tickers), 
...
mMAD.p$cM <- rbind(
  ...
  cbind(
    matrix(
      returns - matrix(rep(mMAD.p$mu, mMAD.p$scenarios), mMAD.p$scenarios), mMAD.p$scenarios
...
mMAD.p$time <- system.time(
  mMAD.p$result <- Rglpk_solve_LP(
    ...
    bounds = list(lower = list(ind = 1:mMAD.p$tickers,
                               val = rep(mMAD.p$lL, mMAD.p$tickers)),
                  upper = list(ind = 1:mMAD.p$tickers,
                               val = rep(mMAD.p$uL, mMAD.p$tickers))),
    rhs = c(mMAD.p$leverage, mMAD.p$pL, rep(0, 2 * mMAD.p$scenarios), rep(-0.005, 2*mMAD.p$tickers)),
    types = c(rep("C", mMAD.p$tickers + mMAD.p$scenarios), rep("B", mMAD.p$tickers)),
    max = FALSE
  ))
